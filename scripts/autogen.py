    ...
    def make_md_source_for_entry(self, entry, path_stack, title_stack):
        ...
        if generate:
            ...
            for element in generate:
                # Add aliases before the element if they exist
                if element in aliases:
                    alias_list = aliases[element]
                    if alias_list:
                        generated_md += "\n**Aliases:**\n\n"
                        for alias in alias_list:
                            generated_md += f"* `{alias}`\n"
                        generated_md += "\n"
    ...
    def make_symbol_to_link_map(self):
        def recursive_make_map(entry, current_url):
            ...
            if "generate" in entry:
                ...
                # Add aliases to the map, pointing to their canonical symbol
                aliases = entry.get("aliases", {})
                for canonical_symbol, alias_list in aliases.items():
                    if canonical_symbol in entry_map:
                        for alias in alias_list:
                            entry_map[alias] = entry_map[canonical_symbol]
