...
class Router(layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        position_in_expert = tf.cast(
            tf.cumsum(expert_mask, axis=0) * expert_mask, "int32"
        )
        # Keep only tokens that fit within expert capacity.
        expert_mask *= tf.cast(
            tf.less(tf.cast(position_in_expert, "int32"), self.expert_capacity),
            "float32",
        )
        ...
        combined_tensor = tf.expand_dims(
            expert_gate
            * expert_mask_flat
            * tf.squeeze(tf.one_hot(expert_index, self.num_experts), 1),
            -1,
