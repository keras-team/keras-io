...
class Dataset:
    ...
    def prepare_mlm_input_and_labels(self, X):
        ...
        X_mlm[inp_mask_2mask] = self.tokenizer.mask_token_id  # mask token is the last in the dict

        # set 10% to a random token
        inp_mask_2random = inp_mask_2mask  & (np.random.rand(*X.shape) < 1/9)
        X_mlm[inp_mask_2random] = np.random.randint(3, self.tokenizer.mask_token_id, inp_mask_2random.sum())
...
class MaskedLanguageModel:
    ...
    def __init__(self, strategy, model_name):
        ...
        self.compute_mlm_loss, self.train_mlm_loss_metric = self.get_mlm_loss_and_metrics()
