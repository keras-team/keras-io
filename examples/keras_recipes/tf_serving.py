...
Here we will load a pre-trained [MobileNet](https://arxiv.org/abs/1704.04861) from the [Keras applications](https://keras.io/api/applications/), this is the model that we are going to serve.
"""
...
print(
    f"Preprocessed image pixel range: ({preprocess_img.numpy().min()}, {preprocess_img.numpy().max()})"
)
...
model_dir = "model"
model_version = 1
model_export_path = f"{base_dir}/{model_dir}/{model_version}"
...
"""
That tells us a lot about our model! For instance, we can see that its inputs have a 4-d shape `(-1, 224, 224, 3)` which means `(batch_size, height, width, channels)`, also note that this model requires a specific image shape `(224, 224, 3)` this means that we may need to reshape our images before sending them to the model. We can also see that the model's outputs have a `(-1, 1000)` shape which are the logits for the 1000 classes of the [ImageNet](https://www.image-net.org) dataset.
...
os.environ["MODEL_DIR"] = f"{base_dir}/{model_dir}"
...
def predict_grpc(data, input_name, stub):
    ...
    # Set which signature is used to format the gRPC query, here the default one "serving_default"
...
model_sig_version = 2
model_sig_export_path = f"{base_dir}/{model_dir}/{model_sig_version}"
...
"""
Note that this model has a different signature, its input is still 4-d but now with a `(-1, -1, -1, 3)` shape, which means that it supports images with any height and width size. Its output also has a different shape, it no longer outputs the 1000-long logits.
