...
how to build a ranking model for the Movielens dataset score movies to suggest
to users.
...
Let's begin by choosing Tensorflow as the backend and importing all the
necessary libraries.
...
We need to distinguish between the batch size that each TPU will see and the
global batch size that is consumed at each step.


```python
PER_REPLICA_BATCH_SIZE = 256
GLOBAL_BATCH_SIZE = PER_REPLICA_BATCH_SIZE * jax.local_device_count("tpu")
```
...
We need do know the number of users as we're using the user ID directly as an
...
train_ratings = (
    shuffled_ratings.take(80_000).batch(GLOBAL_BATCH_SIZE, drop_remainder=True).cache()
)
test_ratings = (
...
embedding lookups into a single invocations. In this section, we'll describe
...
- A placement to tell whether to put the table on the SparseCore chips or not.
  In this case, will want the `"sparsecore"` placement.
...
FEATURE_CONFIGS = {
    "movie_id": keras_rs.layers.FeatureConfig(
        ...
        output_shape=(GLOBAL_BATCH_SIZE, EMBEDDING_DIMENSION),
    ),
...
Let's now instantiate the model. Note that `DistributedEmbedding` needs to be
instantiated within the `TPUStrategy`, so we simply create the whole model under
the strategy.

We then use `model.compile()` to configure the loss, metrics and optimizer.
Again, this Adagrad optimizer will only apply to the dense layers and not the
embedding tables.
...
hardward-dependent format required for use with SparseCores. We'll do this by
...
312/312 ━━━━━━━━━━━━━━━━━━━━ 14s 37ms/step - loss: 0.2829 - root_mean_squared_error: 0.5283

Epoch 2/5

312/312 ━━━━━━━━━━━━━━━━━━━━ 2s 7us/step - loss: 0.0936 - root_mean_squared_error: 0.3059

Epoch 3/5

312/312 ━━━━━━━━━━━━━━━━━━━━ 0s 9us/step - loss: 0.0933 - root_mean_squared_error: 0.3055

Epoch 4/5

312/312 ━━━━━━━━━━━━━━━━━━━━ 0s 8us/step - loss: 0.0931 - root_mean_squared_error: 0.3051

Epoch 5/5

312/312 ━━━━━━━━━━━━━━━━━━━━ 0s 8us/step - loss: 0.0928 - root_mean_squared_error: 0.3047

<keras.src.callbacks.history.History at 0x76e69775bc10>
```
</div>
...
<div class="k-default-codeblock">
```
78/78 ━━━━━━━━━━━━━━━━━━━━ 2s 21ms/step - loss: 0.0999 - root_mean_squared_error: 0.3161

{'loss': 0.10089830309152603, 'root_mean_squared_error': 0.3176449239253998}
```
</div>
