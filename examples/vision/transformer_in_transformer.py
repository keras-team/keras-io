...
Description: Implementing the Transformer in Transformer (TNT) model for image
classification.
"""
...
def pixel_embed(x, image_size=96, patch_size=8, in_dim=48, stride=4):
    ...
    x = tf.reshape(x, shape=(-1, in_dim, new_patch_size[0], new_patch_size[1]))
    # pixel position embedding
    pixel_position = tf.Variable(
        tf.zeros(shape=(1, in_dim, new_patch_size[0], new_patch_size[1]))
    )
    x = layers.add([x, pixel_position])
...
def get_model(
    image_size=96,
    patch_size=8,
    out_embedding_dim=32,  # outer transformer block embedding dim
    in_embedding_dim=16,  # inner transformer block embedding dim
    ...
    pixel_embedding, num_patches, new_patch_size = pixel_embed(
        x, image_size, patch_size, in_embedding_dim, first_stride
    )
    num_pixel = new_patch_size[0] * new_patch_size[1]
    patch_position = tf.Variable(tf.zeros(shape=(1, num_patches, out_embedding_dim)))
    # extract patch embedding
    patch_embedding = tf.reshape(
        pixel_embedding, shape=(-1, num_patches, in_embedding_dim * num_pixel)
    )
    patch_embedding = layers.LayerNormalization(epsilon=1e-5)(patch_embedding)
    patch_embedding = layers.Dense(out_embedding_dim)(patch_embedding)
    patch_embedding = layers.LayerNormalization(epsilon=1e-5)(patch_embedding)
    patch_embedding = layers.add([patch_embedding, patch_position])
...
"""
After 30 epochs, the TNT model achieves around 35% accuracy and
66% top-5 accuracy on the test data. It only has 0.1M parameters.
