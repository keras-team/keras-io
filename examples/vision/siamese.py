...
def make_pairs(x, y):
    """
    Creates a tuple containing image pairs with coreesponding label
...
def visualize(pairs, labels, to_show=6, num_col=3, predictions=None, test=False):
    """
    Creates a plot pairs with label and prediction if it's test dataset
...
visualize(pairs_train, labels_train, to_show=3)
...
siamese.fit(
    [x_train_1, x_train_2], labels_train, batch_size=16, validation_split=0.1, epochs=6
)
...
visualize(
    pairs_test, labels_test, to_show=3, num_col=3, predictions=predictions, test=True
)
